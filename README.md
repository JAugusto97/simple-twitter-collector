...
3. (Optional) Follow these [instructions](https://developers.google.com/drive/api/v3/about-auth#OAuth2Authorizing) to get your Google Drive OAuth 2.0 secret keys. Make sure to select "Computer App" as the type for your keys. Download the **client_secrets.json** file and place it into **credentials/client_secrets.json**
...
6. To collect the tweets:  
   ```make collect```
7. (Optional) To start a PostgreSQL database:
8. ```make startdb```
9.  (Optional) To parse the raw tweets into a PostgreSQL database:  
  ```make parse```
